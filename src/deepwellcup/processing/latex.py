...
class Latex:
    ...
    def _make_table_contents(self):
        ...
        ranking_image_path = (
            f"../../figures/{self.year}/Points-{self.year}-Round{previous_round}.pdf"
        )
    ...
    def _create_row(self, series):
        ...
        return self._row_team_selection(series) + self._row_player_selection(series)
    ...
    def _make_scoring_table(self):
        ...
        return template.render(
            column_format="l l", scoring_table=self._scoring_table_contents()
        )
    ...
    def _counts_table_teams(self):
        ...
        def team_counts_string(team):
            counts = picks_per_team[ltn(team)] if ltn(team) in picks_per_team else 0
    ...
    def _counts_table_players(self):
        ...
        for higher_seed, lower_seed in players:
            ...
            lower_counts = (
                picks_per_player[lower_seed] if lower_seed in picks_per_player else 0
            )
    ...
    def _points_table(self, func):
        ...
        & & 4 & 5 & 6 & 7\\\cline{2-6}"""
            ...
            + r"\\"
            + "\n"
            + f"        & 6 & {self._make_points_string(func, 6)}"
            + r"\\"
            + "\n"
